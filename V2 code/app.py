...
def generate_payslip_pdf(employee_data, timesheet_df, pay_period_end, country=None):
    ...
    if os.path.exists(logo_path):
        try:
            # Use ReportLab's direct image loading without PIL conversion
            logo_width = 220  # Increased logo width for bigger size
            logo_height = 120  # Increased logo height for bigger size
            logo_x = width - margin - logo_width - 10  # Position from right edge
            logo_y = y - logo_height - 10  # Position from top
            
            c.drawImage(logo_path, logo_x, logo_y, width=logo_width, height=logo_height, preserveAspectRatio=True, mask='auto')
        except Exception as e:
            # Fallback: try with PIL conversion
            try:
                from PIL import Image
                img = Image.open(logo_path)
                temp_path = "temp_logo.png"
                img.save(temp_path, "PNG")
                c.drawImage(temp_path, width - margin - 180, y - 100, width=180, height=90, preserveAspectRatio=True)
                os.remove(temp_path)
            except:
                pass
    
    # Move down to give space for logo
    ...
    header_data = [
        ...
        ["EMPLOYEE NO.", employee_data['employee_id'], "NAME", employee_data['employee_name']],
        ["EMPLOYMENT", employee_data['employment_type'], "PAY DATE", pay_period_str]
    ]
